...
contract Staking{
    ...
    modifier onlyOwner  {
        require (msg.sender == owner,"only owner can do this function");
        _;
    }
    ...
    function beginStaking(uint _cutoffTime) external onlyOwner {
        require(rewardsToken.balanceOf(msg.sender)>=totalRewardSupply, "not enough staking rewards");
        active = true;
    ...
    require(depositAmount>0,"Amount should be greater than zero");
    require(stakingToken.balanceOf(msg.sender) >= depositAmount, "Insufficient Balance Your Account");
